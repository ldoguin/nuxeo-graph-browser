...
public class MyRoot extends ModuleRoot {
	...
	public Object getChoices(@PathParam("uuid") String uuid) throws Exception {
		...
		predicateJSObject = createPredicateNode("parentOf",
				doc.getId());
		...
		// add tags Navigation
		statementJSObject = new JSONObject();
		predicateJSObject = createPredicateNode("hasTags",
				doc.getId());
		...
		statementJSObject = new JSONObject();
		predicateJSObject = createPredicateNode("hasRelations",
				doc.getId());
	...
	private JSONObject createDocumentNode(String label, String uuid,
		...
		JSONObject docNode = createJsonNode(50, color, label, "doc", 70,
				uuid);
